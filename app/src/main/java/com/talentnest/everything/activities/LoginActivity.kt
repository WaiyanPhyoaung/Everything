        ...
        databaseReference.addListenerForSingleValueEvent(object : ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot) {
               if(snapshot.child(parentDbName).child(loginPhoneNumber).exists()){
                   ...
                   val user : UserModel? = snapshot.child(parentDbName).child(loginPhoneNumber).getValue(UserModel::class.java)
                   ...
                   Toast.makeText(this@LoginActivity,"This account doesn't exist...",Toast.LENGTH_SHORT).show()
                   hideDialog()
               }
            }
